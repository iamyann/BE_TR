...
void initStruct(void) {
    ...
    if (err = rt_mutex_create(&mutexCptEchec, NULL)) {
        rt_printf("Error mutex create: %s\n", strerror(-err));
        exit(EXIT_FAILURE);
    }
    if (err = rt_mutex_create(&mutexStatus, NULL)) {
        rt_printf("Error mutex create: %s\n", strerror(-err));
        exit(EXIT_FAILURE);
    }
    ...
    if (err = rt_task_create(&tBattery, NULL, 0, PRIORITY_TBAT, 0)) {
        rt_printf("Error task create: %s\n", strerror(-err));
        exit(EXIT_FAILURE);
    }
...
void startTasks() {
    ...
    if (err = rt_task_start(&tBattery, &get_etat_battery, NULL)) {
        rt_printf("Error task start: %s\n", strerror(-err));
        exit(EXIT_FAILURE);
    }
